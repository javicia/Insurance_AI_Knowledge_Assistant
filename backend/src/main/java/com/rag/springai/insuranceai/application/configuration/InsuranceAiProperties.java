...
public record InsuranceAiProperties(Rag rag, Security security, Governance governance, Ai ai, Evaluation evaluation) {
    ...
    public record Security(PromptInjection promptInjection, Pii pii, OAuth2 oauth2) {
        ...
        /**
         * FASE 17 (IAM). {@code issuer} is validated against the JWT's {@code iss} claim
         * separately from where signing keys are fetched from ({@code spring.security
         * .oauth2.resourceserver.jwt.jwk-set-uri}) - the two are deliberately decoupled because
         * in this Docker Compose topology they are never the same URL: Keycloak issues tokens
         * whose {@code iss} claim reflects the externally-visible address the browser used
         * ({@code http://localhost:8180/realms/insurance-ai}), while this backend fetches
         * signing keys over the internal Docker network ({@code http://keycloak:8080/...}) -
         * see {@code adapters.inbound.security.SecurityConfiguration#jwtDecoder} and
         * {@code docs/security/IAM_ARCHITECTURE.md}.
         */
        public record OAuth2(String issuer) {
            public OAuth2 {
                Objects.requireNonNull(issuer, "insurance-ai.security.oauth2.issuer must be configured");
            }
        }
    }
